...
dirty money you then have to run through `gtarp_laundering` before it spends
...
- **Distinct from `gtarp_fightclub`.** Fightclub is *parimutuel* wagering on a
...
- **Feeds the dirty-money economy.** Winnings pay in `black_money` (same item
  `qbx_bankrobbery` drops and `gtarp_laundering` washes) — so a jackpot is a
  new *source* of dirty money that flows into the laundering sink. Never
  `counterfeit_cash` (that's `gtarp_counterfeit`'s fake-money lane) and never
...
- The bet cooldown is set **before** any DB yield (the `gtarp_chopshop` rl()
...
`gtarp_numbers_bets` (one row per slip: number, stake, draw_seq, status,
payout, paid) and `gtarp_numbers_draws` (one row per resolved draw: winning
