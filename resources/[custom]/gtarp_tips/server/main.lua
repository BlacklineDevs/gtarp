...
-- police 911 log via gtarp_mdt's LogCall export with NO identity
...
local function dbg(msg)
    if Config.Debug then print('[gtarp_tips] ' .. msg) end
end
...
local function cmdTip(src, args)
    ...
    local logged = false
    pcall(function()
        logged = exports.gtarp_mdt:LogCall(Config.Tip.Prefix .. text, phone, 'anonymous') == true
    end)
    if not logged then
        Bridge.Notify(src, 'Payphone', 'The line is dead.', 'error')
        return
...
AddEventHandler('onResourceStart', function(resource)
    ...
    print(('[gtarp_tips] tip line open — %d payphone(s); 911 log %s')
...
---tips live in gtarp_mdt's call log, anonymously).
