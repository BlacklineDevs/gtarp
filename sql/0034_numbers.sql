-- 0034_numbers.sql — tables for gtarp_numbers. Apply after the qbx base schema.
...
CREATE TABLE IF NOT EXISTS `gtarp_numbers_bets` (
    ...
    INDEX idx_gtarp_numbers_bets_draw (draw_seq, status),
    INDEX idx_gtarp_numbers_bets_collect (citizenid, status, paid)
);

-- One row per resolved draw — the results history + volume ledger.
CREATE TABLE IF NOT EXISTS `gtarp_numbers_draws` (
